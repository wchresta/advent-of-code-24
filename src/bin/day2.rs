...
fn part1(content: &Vec<Vec<i32>>) -> usize {
    let diffs: Vec<Vec<i32>> = content.iter().map(|r: &Vec<i32>| zip(r.iter(), r.iter().skip(1)).map(|(x,y)| y-x).collect()).collect();
...
fn part2(content: &Vec<Vec<i32>>) -> usize {
    ...
    for report in content {
        for i in 0..report.len() {
            ...
            let diffs = zip(cut_report.iter(), cut_report.iter().skip(1)).map(|(x,y)| y-x).collect();
            if is_safe(&diffs) {
                count += 1;
                break;
            }
